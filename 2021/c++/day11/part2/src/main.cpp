    ...
    for (unsigned int i = 0; i < cavern.size(); ++i) {
      ...
      for (unsigned int j = 0; j < ROW_LENGTH; ++j) {
        ...
        if (isNorthWest(*octopus)) {
          ...
          // std::cout << "Neighbors for " << octopus->idx << std::endl;
          // for (auto neighbor : *(octopus->neighbors)) {
